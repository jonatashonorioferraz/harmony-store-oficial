...
export async function workflowStatus(db: D1DatabasePort, workflowId: string) {
  ...
  const images = approved ? [1,2,3,4,5].map((n) => latest.get(`visual-production-${n}`)?.output?.candidateAssetId).filter(Boolean).map((id) => ({ id, url: `/api/studio/assets?id=${encodeURIComponent(id)}` })) : [];
