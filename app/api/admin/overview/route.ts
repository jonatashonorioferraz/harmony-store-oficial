...
export async function GET() {
  try {
    ...
    const [knowledge, excellence, settings, events, projects, palettes, visualReferences, visualStandards] = await Promise.all([
      ...
      db.prepare("SELECT id, category, shot_type, version, status, purpose, specification_json, source_reference_ids_json, change_reason, created_at, published_at FROM studio_visual_standard_versions ORDER BY category, shot_type, version DESC").all(),
    ]);
