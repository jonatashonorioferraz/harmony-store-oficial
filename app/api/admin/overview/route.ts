...
export async function GET() {
  try {
    ...
    const [knowledge, excellence, settings, events, projects, palettes, visualReferences] = await Promise.all([
      ...
      db.prepare("SELECT id, title, scope, category, model_name, shot_type, transfer_mode, guidance, never_do_json, status, created_at FROM studio_visual_references ORDER BY created_at DESC LIMIT 100").all(),
    ]);
    const parse = (value: unknown) => typeof value === "string" ? JSON.parse(value) : value;
    return Response.json({ user, knowledge: (knowledge.results ?? []).map((row: any) => ({ ...row, content: parse(row.content_json), content_json: undefined })), excellence: (excellence.results ?? []).map((row: any) => ({ ...row, tags: parse(row.tags_json), tags_json: undefined })), settings: (settings.results ?? []).map((row: any) => ({ ...row, value: parse(row.value_json), value_json: undefined })), events: events.results ?? [], projects: projects.results ?? [], palettes: (palettes.results ?? []).map((row: any) => ({ ...row, name: row.palette_name, options: parse(row.options_json), palette_name: undefined, options_json: undefined })), visualReferences: (visualReferences.results ?? []).map((row: any) => ({ ...row, neverDo: parse(row.never_do_json), never_do_json: undefined, imageUrl: `/api/admin/visual-references/asset?id=${encodeURIComponent(row.id)}` })) });
