...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === "consolidate") {
      ...
      await db.prepare("INSERT INTO studio_visual_standard_versions (id, category, shot_type, version, status, purpose, specification_json, source_reference_ids_json, reason, created_by, created_at, published_at, archived_at) VALUES (?, ?, ?, ?, 'draft', ?, ?, ?, ?, ?, ?, NULL, NULL)").bind(id, category, shotType, version, `Padrão Harmony para ${shotType}`, JSON.stringify(specification), JSON.stringify(references.map((row) => row.id)), String(body.reason ?? "Consolidação de referências aprovadas"), user.id, now).run();
