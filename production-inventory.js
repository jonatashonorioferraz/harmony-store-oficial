...
async function downloadLabelPng(entry,canvas,button){
  const original=button.innerHTML;button.disabled=true;button.textContent='Gerando PNG…';try{await recordLabelOutput(entry,'png');const blob=await canvasBlob(canvas),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=`Harmony-${entry.box_code||boxCode(entry.box_number)}-150x100-paisagem.png`;document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),30000);toast('Etiqueta PNG gerada em 150 × 100 mm, no formato paisagem.')}catch(error){alert(error.message)}finally{button.disabled=false;button.innerHTML=original}
}
...
async function downloadLabelPdf(entry,canvas,button){
