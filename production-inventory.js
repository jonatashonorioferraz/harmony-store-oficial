...
async function load(force=false){
  ...
  PI.loading=(async()=>{
    const [models,colors,workers,balance,movements,byWorker]=await Promise.all([
      ...
      rpc('list_production_inventory_movements',{p_from:PI.from||null,p_to:PI.to||null,p_worker_id:null,p_model_id:null,p_color_id:null}),
...
async function render(){
  ...
  try{
    ...
    page.innerHTML=`<div class="page production-inventory-page">${head('CONTROLE INTERNO','Inventário de Produção','Saldo dos mini sabonetes acabados, com origem por colaboradora, caixa e data. Não altera pagamentos nem matérias-primas.','<button class="outline" data-inventory-print="balance">📄 Relatório PDF</button><button class="primary" id="newInventoryEntry">＋ Registrar entrada</button>')}${tabs()}${body}</div>`;
    bind();
...
function entryModal(){
  ...
  form.onsubmit=async event=>{event.preventDefault();const button=event.submitter,data=new FormData(form),quantity=Number(data.get('quantity'));if(!Number.isInteger(quantity)||quantity<1)return alert('Informe uma quantidade inteira maior que zero.');button.disabled=true;try{await rpc('create_production_inventory_entry',{p_model_id:data.get('model_id'),p_color_id:data.get('color_id'),p_worker_id:data.get('worker_id'),p_quantity:quantity,p_entry_on:data.get('entry_on'),p_box_reference:data.get('box_reference')||null,p_notes:data.get('notes')||null});closeModal();await refresh('Entrada registrada com rastreabilidade completa.')}catch(error){alert(error.message);button.disabled=false}};
}

async function openDetail(modelId,colorId){
  document.querySelector('#modal').innerHTML='<div class="modal"><section class="modal-box large"><div class="loading-inline">Carregando lotes…</div></section></div>';
  try{
    const entries=await rpc('list_production_inventory_entries',{p_model_id:modelId,p_color_id:colorId,p_include_depleted:true}),row=PI.balance.find(item=>item.model_id===modelId&&item.color_id===colorId),available=entries.filter(entry=>n(entry.current_quantity)>0),byWorker=new Map();
    ...
    document.querySelector('#modal').innerHTML=`<div class="modal"><section class="modal-box large production-inventory-detail" id="inventoryDetail"><div class="modal-head"><div><p class="eyebrow">DETALHE DO ESTOQUE</p><h2>${esc(row?.model_name||entries[0]?.model_name||'Produto')}</h2><span class="production-inventory-color"><i style="--inventory-color:${esc(row?.color_hex||entries[0]?.color_hex||'#D9A3BE')}"></i>${esc(row?.color_name||entries[0]?.color_name||'')}</span></div><div class="actions"><button type="button" class="outline compact-action" id="printInventoryDetail">📄 PDF</button><button type="button" data-inventory-close>×</button></div></div><div class="production-inventory-detail-hero"><span>${row?.image_path?`<img src="${esc(imageUrl(row.image_path))}" alt="">`:'<i>🧼</i>'}</span><div><small>SALDO TOTAL DESTA COR</small><b>${qty(row?.quantity||0)}</b><p>${available.length} lote(s) disponível(is). As saídas são registradas na caixa escolhida.</p></div></div><section class="production-inventory-origin"><h3>Quantidade identificada por colaboradora</h3><div>${[...byWorker.values()].map(item=>`<article><b>${esc(item.name)}</b><span>Produziu ${qty(item.original)}</span><strong>${qty(item.current)} em saldo</strong></article>`).join('')||'<div class="empty">Sem entradas registradas.</div>'}</div></section><section><div class="card-head"><div><p class="eyebrow">LOTES E CAIXAS</p><h2>Histórico detalhado</h2></div></div><div class="production-inventory-lots">${entries.map(entry=>`<article class="${n(entry.current_quantity)?'available':'depleted'}"><header><div><small>LOTE #${String(entry.protocol).padStart(4,'0')}</small><b>${esc(entry.box_reference||'Caixa não informada')}</b></div><span>${n(entry.current_quantity)?'Disponível':'Esgotado'}</span></header><dl><div><dt>Produzido por</dt><dd>${esc(entry.worker_name)}</dd></div><div><dt>Entrada</dt><dd>${br(entry.entry_on)}</dd></div><div><dt>Quantidade inicial</dt><dd>${qty(entry.original_quantity)}</dd></div><div><dt>Saldo do lote</dt><dd><strong>${qty(entry.current_quantity)}</strong></dd></div></dl>${entry.notes?`<p>${esc(entry.notes)}</p>`:''}<footer><small>Registrado por ${esc(entry.created_by_name)} em ${when(entry.created_at)}</small><div class="actions">${n(entry.current_quantity)?`<button class="primary compact-action" data-inventory-exit="${entry.id}">Registrar saída</button>`:''}<button class="outline compact-action" data-inventory-adjust="${entry.id}">Ajustar contagem</button><button class="ghost compact-action" data-inventory-edit-entry="${entry.id}">Editar dados</button></div></footer></article>`).join('')||'<div class="empty">Nenhum lote registrado.</div>'}</div></section></section></div>`;
...
function editMetadataModal(entry,modelId,colorId){
  document.querySelector('#modal').innerHTML=`<div class="modal"><form class="modal-box" id="inventoryMetadataForm"><div class="modal-head"><div><p class="eyebrow">CORREÇÃO AUDITADA</p><h2>Editar dados do lote #${String(entry.protocol).padStart(4,'0')}</h2><span>A quantidade é corrigida somente pela opção Ajustar contagem.</span></div><button type="button" data-inventory-back>×</button></div><div class="form"><label>Produzido por<select name="worker_id" required>${PI.workers.map(worker=>`<option value="${worker.id}" ${worker.id===entry.worker_id?'selected':''}>${esc(worker.full_name)}</option>`).join('')}</select></label><label>Data de entrada<input name="entry_on" type="date" max="${today()}" value="${entry.entry_on}" required></label><label class="wide">Caixa / identificação<input name="box_reference" maxlength="100" value="${esc(entry.box_reference||'')}"></label><label class="wide">Observações<textarea name="notes" maxlength="1200">${esc(entry.notes||'')}</textarea></label><div class="form-actions"><button type="button" class="outline" data-inventory-back>Voltar</button><button class="primary">Salvar correção</button></div></div></form></div>`;
  document.querySelectorAll('[data-inventory-back]').forEach(button=>button.onclick=()=>openDetail(modelId,colorId));document.querySelector('#inventoryMetadataForm').onsubmit=async event=>{event.preventDefault();const button=event.submitter,data=new FormData(event.currentTarget);button.disabled=true;try{await rpc('update_production_inventory_entry_metadata',{p_entry_id:entry.id,p_worker_id:data.get('worker_id'),p_entry_on:data.get('entry_on'),p_box_reference:data.get('box_reference')||null,p_notes:data.get('notes')||null});PI.loaded=false;await load(true);await openDetail(modelId,colorId);toast('Dados do lote corrigidos com auditoria.')}catch(error){alert(error.message);button.disabled=false}};
}
...
function reportHtml(type){
  if(type==='movements')return sheet('Relatório de movimentações',`${PI.from?br(PI.from):'Início'} a ${PI.to?br(PI.to):'Hoje'}`,PI.movements.map(row=>`<tr><td>${br(row.occurred_on)}</td><td><b>${esc(movementLabels[row.movement_type])}</b><small>${esc(row.reason||'')}</small></td><td><b>${esc(row.model_name)}</b><small><i class="swatch" style="--print-color:${esc(row.color_hex)}"></i>${esc(row.color_name)}</small></td><td>${esc(row.worker_name)}<small>${esc(row.box_reference||'Caixa não informada')}</small></td><td>${qty(row.quantity)}</td><td>${qty(row.balance_after)}</td><td>${esc(row.created_by_name)}</td></tr>`).join(''),['Data','Movimento','Modelo / cor','Origem','Quantidade','Saldo lote','Responsável']);
...
function detailHtml(row,entries){return sheet(`${row.model_name} · ${row.color_name}`,'Rastreabilidade por colaboradora, data e caixa',entries.map(entry=>`<tr><td>#${String(entry.protocol).padStart(4,'0')}</td><td><b>${esc(entry.worker_name)}</b></td><td>${br(entry.entry_on)}</td><td>${esc(entry.box_reference||'Não informada')}</td><td>${qty(entry.original_quantity)}</td><td><b>${qty(entry.current_quantity)}</b></td><td>${esc(entry.notes||'')}</td></tr>`).join(''),['Lote','Produzido por','Entrada','Caixa','Inicial','Saldo','Observação'],`<div class="print-summary"><span><small>MODELO</small><b>${esc(row.model_name)}</b></span><span><small>COR</small><b>${esc(row.color_name)}</b></span><span><small>SALDO</small><b>${qty(row.quantity)}</b></span></div>`)}
