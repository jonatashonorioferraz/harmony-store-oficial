...
const LABEL_WIDTH=1200,LABEL_HEIGHT=800,LABEL_TEMPLATE='150x100-landscape-v2';
...
const loadImage=(src,crossOrigin=false)=>new Promise((resolve,reject)=>{const image=new Image();if(crossOrigin)image.crossOrigin='anonymous';image.onload=()=>resolve(image);image.onerror=()=>reject(Error('Não foi possível carregar a imagem.'));image.src=src});
...
async function drawOfficialThermalLogo(ctx,x,y,width,height){
...
async function drawThermalProductPhoto(ctx,path,x,y,size){
  ctx.save();ctx.fillStyle='#fff';ctx.strokeStyle='#000';ctx.lineWidth=2;roundedRect(ctx,x,y,size,size,12);ctx.fill();ctx.stroke();ctx.restore();
  if(!path)return false;
  try{
    const source=await loadImage(imageUrl(path),true),scratch=document.createElement('canvas'),resolution=180;scratch.width=resolution;scratch.height=resolution;const local=scratch.getContext('2d',{willReadFrequently:true}),sourceWidth=source.naturalWidth||source.width,sourceHeight=source.naturalHeight||source.height,sourceSide=Math.min(sourceWidth,sourceHeight),sourceX=(sourceWidth-sourceSide)/2,sourceY=(sourceHeight-sourceSide)/2;
    local.fillStyle='#fff';local.fillRect(0,0,resolution,resolution);local.drawImage(source,sourceX,sourceY,sourceSide,sourceSide,0,0,resolution,resolution);const pixels=local.getImageData(0,0,resolution,resolution),data=pixels.data,bayer=[0,8,2,10,12,4,14,6,3,11,1,9,15,7,13,5];
    for(let row=0;row<resolution;row++)for(let column=0;column<resolution;column++){const index=(row*resolution+column)*4,gray=.299*data[index]+.587*data[index+1]+.114*data[index+2],threshold=118+bayer[(row%4)*4+(column%4)]*8,ink=gray<threshold;data[index]=data[index+1]=data[index+2]=ink?0:255;data[index+3]=255}
    local.putImageData(pixels,0,0);ctx.save();roundedRect(ctx,x+5,y+5,size-10,size-10,9);ctx.clip();ctx.drawImage(scratch,x+5,y+5,size-10,size-10);ctx.restore();return true;
  }catch{return false}
}
async function drawProductModelField(ctx,entry,x,y,width,height){
  ctx.strokeStyle='#000';ctx.lineWidth=2;roundedRect(ctx,x,y,width,height,14);ctx.stroke();const photoSize=height-20;await drawThermalProductPhoto(ctx,entry.image_path,x+10,y+10,photoSize);const textX=x+photoSize+28,max=width-photoSize-46;ctx.fillStyle='#000';ctx.font='700 18px Arial';ctx.fillText('MODELO',textX,y+29);const size=fitCanvasText(ctx,entry.model_name,max,40,23,700,'Arial');ctx.font=`700 ${size}px Arial`;wrapCanvasText(ctx,entry.model_name,max,2).forEach((line,index)=>ctx.fillText(line,textX,y+68+index*(size+4)));
}
async function renderLabelCanvas(entry,canvas){
  canvas.dataset.labelTemplate=LABEL_TEMPLATE;
  canvas.width=LABEL_WIDTH;canvas.height=LABEL_HEIGHT;const ctx=canvas.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,LABEL_WIDTH,LABEL_HEIGHT);ctx.strokeStyle='#000';ctx.lineWidth=6;ctx.strokeRect(12,12,LABEL_WIDTH-24,LABEL_HEIGHT-24);
  await drawOfficialThermalLogo(ctx,38,24,292,180);
  ctx.fillStyle='#000';ctx.textAlign='center';ctx.font='700 25px Arial';ctx.fillText('INVENTÁRIO DE PRODUÇÃO',622,72);ctx.font='900 88px Arial';ctx.fillText(entry.box_code||boxCode(entry.box_number),622,169);
  drawQr(ctx,labelUrl(entry),942,26,216);ctx.font='700 13px Arial';ctx.fillText('ESCANEIE PARA CONSULTAR',1050,260);ctx.fillText('A CAIXA NO APLICATIVO',1050,279);
  ctx.textAlign='left';await drawProductModelField(ctx,entry,42,303,770,127);drawLabelField(ctx,'Quantidade',qty(entry.original_quantity),830,303,328,127,{large:true});drawLabelField(ctx,'Cor',entry.color_name,42,450,365,100);drawLabelField(ctx,'Produzido por',entry.worker_name,425,450,733,100,{wrap:true});drawLabelField(ctx,'Entrada',br(entry.entry_on),42,570,300,92);drawLabelField(ctx,'Localização',entry.box_reference||'Não informada',360,570,798,92,{wrap:true});
  ctx.textAlign='center';ctx.font='700 17px Arial';ctx.fillText('HARMONY STORE OFICIAL · ETIQUETA 150 × 100 MM · PAISAGEM',600,754);ctx.textAlign='left';return canvas;
}
...
async function recordLabelOutput(entry,format,reissueReason=null){
  if(reissueReason?.trim())return rpc('record_production_inventory_label_print',{p_entry_id:entry.id,p_output_format:format,p_reason:reissueReason.trim()});
...
async function downloadLabelPng(entry,canvas,button,reissueReason=null){
  const original=button.innerHTML;button.disabled=true;button.textContent='Gerando PNG…';try{await recordLabelOutput(entry,'png',reissueReason);const blob=await canvasBlob(canvas),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=`Harmony-${entry.box_code||boxCode(entry.box_number)}-150x100-paisagem.png`;document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),30000);toast(reissueReason?'Etiqueta substituta registrada e gerada em PNG.':'Etiqueta PNG gerada em 150 × 100 mm, no formato paisagem.')}catch(error){alert(error.message)}finally{button.disabled=false;button.innerHTML=original}
}
function downloadLabelFile(blob,filename){
...
async function downloadLabelPdf(entry,canvas,button,reissueReason=null){
  const original=button.innerHTML;button.disabled=true;button.textContent='Gerando PDF de 1 página…';try{
    if(!window.HarmonyThermalPdf)throw Error('O gerador do PDF térmico não foi carregado. Atualize o aplicativo e tente novamente.');
    const blob=await window.HarmonyThermalPdf.createPdfBlobFromCanvas(canvas,{widthMm:150,heightMm:100,quality:.98});
    await recordLabelOutput(entry,'pdf',reissueReason);
    downloadLabelFile(blob,`Harmony-${entry.box_code||boxCode(entry.box_number)}-150x100-paisagem.pdf`);
    toast(reissueReason?'Etiqueta substituta registrada e gerada em PDF.':'PDF térmico gerado: 1 página exata de 150 × 100 mm, em paisagem.');
  }catch(error){alert(error.message)}finally{button.disabled=false;button.innerHTML=original}
}
async function printThermalLabel(entry,canvas,button,reissueReason=null){
  const original=button.innerHTML;button.disabled=true;button.textContent='Preparando impressão…';let pageStyle;try{await recordLabelOutput(entry,'print',reissueReason);const root=document.createElement('main');root.id='productionInventoryLabelPrintRoot';root.innerHTML=`<img src="${canvas.toDataURL('image/png')}" alt="Etiqueta ${esc(entry.box_code||boxCode(entry.box_number))}">`;pageStyle=document.createElement('style');pageStyle.id='productionInventoryLabelPageSize';pageStyle.textContent='@media print{@page{size:150mm 100mm;margin:0}html,body,#productionInventoryLabelPrintRoot{break-after:avoid-page!important;break-inside:avoid-page!important;page-break-after:avoid!important;page-break-inside:avoid!important}}';document.head.appendChild(pageStyle);document.body.appendChild(root);await waitAssets(root);await window.HarmonyPrint.printCurrentDocument('production-inventory-label-printing',()=>{root.remove();pageStyle.remove()})}catch(error){document.querySelector('#productionInventoryLabelPrintRoot')?.remove();pageStyle?.remove();alert(error.message)}finally{button.disabled=false;button.innerHTML=original}
}
...
function labelReissueModal(entry,modelId,colorId){
  const reasons=[['damaged','Etiqueta danificada'],['legacy','Caixa anterior ao sistema de etiquetas'],['illegible','Etiqueta ilegível ou desbotada'],['lost','Etiqueta perdida ou removida'],['other','Outro motivo']];
  document.querySelector('#modal').innerHTML=`<div class="modal"><form class="modal-box" id="inventoryLabelReissueForm"><div class="modal-head"><div><p class="eyebrow">REEMISSÃO AUDITADA</p><h2>Gerar etiqueta da caixa ${esc(entry.box_code||boxCode(entry.box_number))}</h2><span>O código permanente e o QR Code serão preservados. Nenhum saldo ou movimento será alterado.</span></div><button type="button" data-inventory-reissue-back>×</button></div><div class="form"><label class="wide">Motivo<select name="reason" required><option value="">Selecione o motivo</option>${reasons.map(([value,label])=>`<option value="${value}">${label}</option>`).join('')}</select></label><label class="wide">Observação complementar<textarea name="details" maxlength="500" placeholder="Opcional, exceto quando escolher Outro motivo"></textarea></label><aside class="production-inventory-label-safety wide"><b>🔒 Reemissão segura</b><span>Esta ação gera somente uma nova via da etiqueta existente e registra motivo, usuário e horário na auditoria.</span></aside><div class="form-actions"><button type="button" class="outline" data-inventory-reissue-back>Voltar</button><button class="primary">Continuar para a etiqueta</button></div></div></form></div>`;
  const back=()=>openDetail(modelId,colorId);document.querySelectorAll('[data-inventory-reissue-back]').forEach(button=>button.onclick=back);document.querySelector('#inventoryLabelReissueForm').onsubmit=event=>{event.preventDefault();const data=new FormData(event.currentTarget),key=data.get('reason'),details=String(data.get('details')||'').trim(),label=reasons.find(item=>item[0]===key)?.[1];if(!label)return alert('Selecione o motivo da reemissão.');if(key==='other'&&details.length<5)return alert('Descreva o motivo da reemissão.');const reason=details?`${label}: ${details}`:label;openLabelModal(entry,{reissueReason:reason,back})};
}

async function openLabelModal(entry,{reissueReason=null,back=null}={}){
  document.querySelector('#modal').innerHTML=`<div class="modal"><section class="modal-box large production-inventory-label-modal"><div class="modal-head"><div><p class="eyebrow">${reissueReason?'REEMISSÃO DA ETIQUETA':'IDENTIFICAÇÃO DA CAIXA'}</p><h2>Etiqueta térmica ${esc(entry.box_code||boxCode(entry.box_number))}</h2><span>Formato paisagem 150 × 100 mm · impressão térmica em alto contraste</span></div><button type="button" data-inventory-label-back>×</button></div><div class="production-inventory-label-layout"><div class="production-inventory-label-preview"><canvas id="inventoryThermalLabel" role="img" aria-label="Prévia da etiqueta térmica da caixa ${esc(entry.box_code||boxCode(entry.box_number))}"></canvas><span id="inventoryLabelLoading">Preparando etiqueta oficial…</span></div><aside><div class="production-inventory-label-status ${entry.label_status==='applied'?'applied':'pending'}"><b>${reissueReason?'🔄 Nova via da etiqueta':entry.label_status==='applied'?'✓ Etiqueta aplicada':'⚠ Aguardando aplicação física'}</b><span>${reissueReason?'A etiqueta será reemitida sem alterar a caixa ou seu estoque.':entry.label_status==='applied'?'A caixa já está liberada no saldo do inventário.':'A caixa ainda não altera o saldo do inventário.'}</span></div>${reissueReason?`<p><b>Motivo registrado:</b> ${esc(reissueReason)}</p>`:''}<dl><div><dt>Modelo</dt><dd>${esc(entry.model_name)}</dd></div><div><dt>Cor</dt><dd>${esc(entry.color_name)}</dd></div><div><dt>Quantidade</dt><dd>${qty(entry.original_quantity)}</dd></div><div><dt>Produzido por</dt><dd>${esc(entry.worker_name)}</dd></div><div><dt>Localização</dt><dd>${esc(entry.box_reference||'Não informada')}</dd></div></dl><p>O QR Code contém apenas um identificador opaco. O acesso aos dados continua exigindo login e permissão de ADM ou Recebimento.</p><p class="production-inventory-label-print-tip"><b>PDF pronto para a impressora:</b> use Gerar PDF e abra o arquivo baixado. Ele contém uma única página paisagem de 150 × 100 mm, sem cabeçalhos do Gmail.</p></aside></div><div class="production-inventory-label-actions"><button type="button" class="outline" id="downloadInventoryLabelPng" disabled>Baixar PNG</button><button type="button" class="outline" id="pdfInventoryLabel" disabled>Gerar PDF 150 × 100</button><button type="button" class="primary" id="printInventoryLabel" disabled>Imprimir etiqueta</button>${entry.label_status!=='applied'?'<button type="button" class="inventory-label-confirm" id="confirmInventoryLabel" disabled>✓ Confirmar etiqueta aplicada</button>':''}</div></section></div>`;
  document.querySelectorAll('[data-inventory-label-back]').forEach(button=>button.onclick=back||closeModal);const canvas=document.querySelector('#inventoryThermalLabel'),loading=document.querySelector('#inventoryLabelLoading'),buttons=[document.querySelector('#downloadInventoryLabelPng'),document.querySelector('#pdfInventoryLabel'),document.querySelector('#printInventoryLabel'),document.querySelector('#confirmInventoryLabel')].filter(Boolean);try{await renderLabelCanvas(entry,canvas);loading.remove();buttons.forEach(button=>button.disabled=false);document.querySelector('#downloadInventoryLabelPng').onclick=event=>downloadLabelPng(entry,canvas,event.currentTarget,reissueReason);document.querySelector('#pdfInventoryLabel').onclick=event=>downloadLabelPdf(entry,canvas,event.currentTarget,reissueReason);document.querySelector('#printInventoryLabel').onclick=event=>printThermalLabel(entry,canvas,event.currentTarget,reissueReason);document.querySelector('#confirmInventoryLabel')?.addEventListener('click',event=>confirmLabelApplied(entry,event.currentTarget))}catch(error){loading.textContent=error.message;loading.classList.add('error')}
}
