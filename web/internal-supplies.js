...
const localDateTimeValue=value=>{const date=new Date(value||Date.now()),valid=Number.isFinite(date.getTime())?date:new Date(),local=new Date(valid.getTime()-valid.getTimezoneOffset()*60000);return local.toISOString().slice(0,16)};
const receiptDateDistance=value=>{const date=new Date(value),today=new Date();if(!Number.isFinite(date.getTime()))return 0;return Math.round((Date.UTC(date.getFullYear(),date.getMonth(),date.getDate())-Date.UTC(today.getFullYear(),today.getMonth(),today.getDate()))/86400000)};
const unusualReceiptDate=value=>Math.abs(receiptDateDistance(value))>7;
const receiptDateAlert=value=>{const date=new Date(value),distance=receiptDateDistance(value),direction=distance<0?'anterior':'posterior';return `A data lida (${date.toLocaleDateString('pt-BR')}) está ${Math.abs(distance)} dias ${direction} a hoje. Confira a data do cupom antes de salvar.`};
...
const fulfillmentByRequestItem=id=>IS.fulfillments.find(item=>item.request_item_id===id);
...
async function loadInternal(force=false){
  ...
  try{
    ...
    if(isAdmin())calls.push(restAll('suppliers?select=*&order=name.asc,id.asc'),rest('internal_receipt_ai_runs?select=*&order=created_at.desc&limit=500'),restAll('internal_supply_request_item_fulfillments?select=*&order=created_at.asc,id.asc'));
    const [requests,requestItemsData,receipts,receiptItemsData,suppliers=[],aiRuns=[],fulfillments=[]]=await Promise.all(calls);
    Object.assign(IS,{products:S.products.filter(product=>product.usage_scope==='internal'),requests,requestItems:requestItemsData,receipts,receiptItems:receiptItemsData,fulfillments,suppliers,aiRuns,loaded:true});
...
function catalog(){
  return `<section class="supply-receipt-head"><div><p class="eyebrow">CATÁLOGO INTERNO</p><h2>Produtos de consumo e operação</h2><span>Estes produtos não aparecem para as colaboradoras de produção.</span></div><button class="primary" id="newSupplyProduct">＋ Cadastrar suprimento</button></section><div class="supply-catalog">${IS.products.map(item=>`<article class="card"><div class="supply-product-icon">${item.image_path?`<img src="${productImageUrl(item.image_path)}" alt="${esc(item.name)}" loading="lazy">`:'🧴'}</div><div><span class="badge ${item.active?'active':'inactive'}">${item.active?'Ativo':'Inativo'}</span><h3>${esc(item.name)}</h3><p>${esc(item.description||item.unit)}</p></div><dl><div><dt>Estoque</dt><dd>${quantity(item.physical_stock,item.unit)}</dd></div><div><dt>Mínimo</dt><dd>${quantity(item.minimum_stock,item.unit)}</dd></div></dl><div class="supply-catalog-actions"><button class="ghost" data-edit-supply-product="${item.id}">Editar</button><button class="danger" data-delete-supply-product="${item.id}">Excluir</button></div></article>`).join('')||'<div class="empty">Nenhum suprimento interno cadastrado.</div>'}</div>`;
}
...
function requestModal(item){
  ...
  const linkedReceiptIds=new Set(linked.map(receipt=>receipt.id));
  document.querySelector('#modal').innerHTML=`<div class="modal"><section class="modal-box supply-modal"><div class="modal-head"><div><p class="eyebrow">SOLICITAÇÃO INTERNA #${String(item.protocol).padStart(4,'0')}</p><h2>${esc(item.requested_by_name)}</h2></div><button data-close>×</button></div><div class="supply-detail-meta"><span><small>Status</small><b class="badge ${item.status}">${statusText[item.status]}</b></span><span><small>Prioridade</small><b>${priorityText[item.priority]}</b></span><span><small>Necessário até</small><b>${item.needed_by?new Date(item.needed_by+'T12:00:00').toLocaleDateString('pt-BR'):'Não informado'}</b></span></div><div class="supply-detail-items">${items.map(row=>{const product=productBy(row.product_id),manual=fulfillmentByRequestItem(row.id),manualReceiptItem=manual&&IS.receiptItems.find(receiptItem=>receiptItem.id===manual.receipt_item_id&&linkedReceiptIds.has(receiptItem.receipt_id)),automatic=linked.some(receipt=>receiptItems(receipt.id).some(receiptItem=>receiptItem.product_id===row.product_id)),bought=automatic||Boolean(manualReceiptItem);return `<article class="has-resolution">${product?.image_path?`<img class="supply-detail-photo" src="${productImageUrl(product.image_path)}" alt="${esc(product.name)}" loading="lazy">`:`<span class="supply-detail-photo supply-detail-photo-fallback">🧴</span>`}<div><b>${esc(product?.name||'Produto')}</b><small>${esc(product?.description||'Item solicitado')}</small>${manualReceiptItem?`<em class="request-item-match-note">Vinculado a “${esc(manualReceiptItem.raw_description)}”</em>`:''}</div><span class="request-item-resolution"><span class="request-item-state ${bought?'bought':'waiting'}">${bought?'✓ Comprado':'Aguardando compra'}</span>${isAdmin()&&linked.length&&!automatic?`<button type="button" class="request-item-link-button" data-match-request-item="${row.id}">${manualReceiptItem?'Revisar vínculo':'Vincular item do cupom'}</button>`:''}</span></article>`}).join('')}</div>${item.notes?`<div class="supply-note"><small>OBSERVAÇÃO</small><p>${esc(item.notes)}</p></div>`:''}${linked.length?`<div class="linked-receipts"><small>CUPONS VINCULADOS</small>${linked.map(receipt=>`<span>🧾 #${String(receipt.protocol).padStart(4,'0')} · ${esc(receipt.merchant_name)} · ${money(receipt.total_value)}</span>`).join('')}</div>`:''}<div class="form-actions supply-actions">${own&&item.status==='pending'?'<button class="outline" id="editInternalRequest">Editar</button><button class="danger" id="cancelOwnInternal">Cancelar</button>':''}${isAdmin()&&['pending','separating'].includes(item.status)?'<button class="primary" id="attachInternalReceipt">📷 Anexar cupom da compra</button>':''}${isAdmin()&&!['delivered','cancelled'].includes(item.status)?'<button class="danger" id="adminCancelInternal">Cancelar solicitação</button>':''}</div></section></div>`;
  document.querySelector('[data-close]').onclick=closeModal;
  document.querySelectorAll('[data-match-request-item]').forEach(button=>button.onclick=()=>receiptMatchModal(item,items.find(row=>row.id===button.dataset.matchRequestItem)));
...
function receiptMatchModal(request,requestItem){
  if(!requestItem||!isAdmin())return;
  const wantedProduct=productBy(requestItem.product_id),linked=IS.receipts.filter(receipt=>receipt.request_id===request.id&&receipt.status==='confirmed'),linkedIds=new Set(linked.map(receipt=>receipt.id)),otherRequestedProducts=new Set(requestItems(request.id).filter(item=>item.id!==requestItem.id).map(item=>item.product_id)),existing=fulfillmentByRequestItem(requestItem.id),usedByOthers=new Set(IS.fulfillments.filter(item=>item.request_item_id!==requestItem.id).map(item=>item.receipt_item_id));
  const candidates=IS.receiptItems.filter(item=>linkedIds.has(item.receipt_id)&&(!usedByOthers.has(item.id)||item.id===existing?.receipt_item_id)&&item.product_id!==requestItem.product_id&&!otherRequestedProducts.has(item.product_id));
  document.querySelector('#modal').innerHTML=`<div class="modal"><form class="modal-box supply-match-modal" id="receiptMatchForm"><div class="modal-head"><div><p class="eyebrow">CONFERÊNCIA DA COMPRA</p><h2>Qual item do cupom corresponde?</h2></div><button type="button" data-back>×</button></div><div class="supply-match-target"><small>ITEM SOLICITADO</small><b>${esc(wantedProduct?.name||'Produto')}</b><span>Escolha abaixo a descrição usada pelo estabelecimento.</span></div><div class="supply-match-options">${candidates.map(item=>{const receipt=linked.find(row=>row.id===item.receipt_id),currentProduct=productBy(item.product_id);return `<label class="supply-match-option"><input type="radio" name="receipt_item_id" value="${item.id}" ${existing?.receipt_item_id===item.id?'checked':''} required><span><b>${esc(item.raw_description)}</b><small>Cupom #${String(receipt?.protocol||'').padStart(4,'0')} · ${quantity(item.quantity,item.unit)} · ${money(item.total_price)}</small><em>Registrado como: ${esc(currentProduct?.name||'Produto do cupom')}</em></span></label>`}).join('')||'<div class="supply-match-empty">Nenhuma linha livre deste cupom pode ser vinculada. Confira se o item foi removido na revisão ou se já atende outro produto.</div>'}</div><label class="supply-match-reason">Motivo da confirmação<textarea name="note" minlength="3" maxlength="500" required>${esc(existing?.note||'Descrição comercial diferente no cupom fiscal')}</textarea><small>O texto original do cupom será preservado na auditoria.</small></label><div class="form-actions">${existing?'<button type="button" class="danger" id="removeReceiptMatch">Remover vínculo</button>':''}<button type="button" class="outline" data-back>Voltar</button><button class="primary" ${candidates.length?'':'disabled'}>Confirmar correspondência</button></div></form></div>`;
  document.querySelectorAll('[data-back]').forEach(button=>button.onclick=()=>requestModal(request));
  document.querySelector('#receiptMatchForm').onsubmit=async event=>{event.preventDefault();const button=event.submitter,form=new FormData(event.currentTarget);button.disabled=true;try{await rpc('admin_link_internal_receipt_item',{p_request_item_id:requestItem.id,p_receipt_item_id:form.get('receipt_item_id'),p_note:form.get('note')});await refreshInternalRequestAfterMatch(request.id,'Item do cupom vinculado e solicitação atualizada.')}catch(error){alert(error.message);button.disabled=false}};
  document.querySelector('#removeReceiptMatch')?.addEventListener('click',async event=>{const reason=prompt('Informe o motivo para remover este vínculo:');if(reason===null)return;event.currentTarget.disabled=true;try{await rpc('admin_unlink_internal_receipt_item',{p_request_item_id:requestItem.id,p_note:reason});await refreshInternalRequestAfterMatch(request.id,'Vínculo removido e situação recalculada.')}catch(error){alert(error.message);event.currentTarget.disabled=false}});
}

async function refreshInternalRequestAfterMatch(requestId,message){
  IS.loaded=false;await loadData();await loadInternal(true);renderInternal(document.querySelector('#page'));requestModal(IS.requests.find(item=>item.id===requestId));toast(message);
}
...
function productImageUrl(path){return `${API}/storage/v1/object/public/product-images/${encodedStoragePath(path)}`}
async function uploadInternalProductPhoto(file){
  const allowedTypes=['image/jpeg','image/png','image/webp'];
  if(!allowedTypes.includes(file.type))throw Error('Use uma foto JPG, PNG ou WebP.');
  const compressed=await compressImage(file);
  if(compressed.size>2097152)throw Error('A foto deve ter no máximo 2 MB.');
  const extension=compressed.type==='image/png'?'png':compressed.type==='image/webp'?'webp':'jpg';
  const path=`${S.profile.id}/${crypto.randomUUID()}.${extension}`;
  const response=await storageFetch('/storage/v1/object/product-images/'+encodedStoragePath(path),{method:'POST',headers:{'Content-Type':compressed.type||'image/jpeg','x-upsert':'false'},body:compressed});
  if(!response.ok)throw Error('Não foi possível enviar a foto do suprimento.');
  return path;
}

function productModal(product={}){
  const photo=product.image_path?`<img src="${productImageUrl(product.image_path)}" alt="${esc(product.name||'Suprimento')}">`:'<span>🧴</span>';
  document.querySelector('#modal').innerHTML=`<div class="modal"><form class="modal-box" id="internalProductForm"><div class="modal-head"><div><p class="eyebrow">SUPRIMENTO INTERNO</p><h2>${product.id?'Editar produto':'Novo produto'}</h2></div><button type="button" data-close>×</button></div><div class="internal-product-photo"><div>${photo}</div><label><b>Foto do suprimento</b><input name="photo" type="file" accept="image/jpeg,image/png,image/webp"><small>JPG, PNG ou WebP · máximo de 2 MB</small></label>${product.image_path?'<label class="check internal-remove-photo"><input name="remove_photo" type="checkbox">Remover foto atual</label>':''}</div><div class="form"><label>Nome<input name="name" value="${esc(product.name||'')}" required></label><label>Unidade<select name="unit">${['unidade','quilo','cm','metro','rolo','garrafa','caixa'].map(unit=>`<option ${product.unit===unit?'selected':''}>${unit}</option>`).join('')}</select></label><label>Estoque físico<input name="physical_stock" type="number" min="0" step=".001" value="${product.physical_stock||0}" required></label><label>Estoque mínimo<input name="minimum_stock" type="number" min="0" step=".001" value="${product.minimum_stock||0}" required></label><label>Fornecedor preferencial<select name="supplier_id"><option value="">Não definido</option>${IS.suppliers.filter(item=>item.active).map(item=>`<option value="${item.id}" ${item.id===product.supplier_id?'selected':''}>${esc(item.name)}</option>`).join('')}</select></label><label class="check"><input name="active" type="checkbox" ${product.active!==false?'checked':''}>Produto ativo</label><label class="wide">Descrição<textarea name="description">${esc(product.description||'')}</textarea></label><div class="form-actions"><button type="button" class="outline" data-close>Cancelar</button><button class="primary">Salvar suprimento</button></div></div></form></div>`;
  document.querySelectorAll('[data-close]').forEach(button=>button.onclick=closeModal);
  const input=document.querySelector('#internalProductForm [name="photo"]');
  input.onchange=()=>{const file=input.files[0];if(!file)return;const preview=document.querySelector('.internal-product-photo>div');preview.innerHTML=`<img src="${URL.createObjectURL(file)}" alt="Prévia da nova foto">`};
  document.querySelector('#internalProductForm').onsubmit=async event=>{
    event.preventDefault();const button=event.submitter,form=new FormData(event.target);button.disabled=true;
    let uploadedPath='',nextImage=form.get('remove_photo')==='on'?null:(product.image_path||null);
    try{
      const file=form.get('photo');
      if(file?.size){uploadedPath=await uploadInternalProductPhoto(file);nextImage=uploadedPath}
      await rpc('admin_save_internal_supply_product_v2',{p_product_id:product.id||null,p_name:String(form.get('name')),p_unit:String(form.get('unit')),p_description:String(form.get('description')||''),p_minimum_stock:num(form.get('minimum_stock')),p_physical_stock:num(form.get('physical_stock')),p_supplier_id:form.get('supplier_id')||null,p_active:form.get('active')==='on',p_image_path:nextImage});
      if(product.image_path&&product.image_path!==nextImage)storageFetch('/storage/v1/object/product-images/'+encodedStoragePath(product.image_path),{method:'DELETE'}).catch(()=>{});
      closeModal();IS.loaded=false;await loadData();await loadInternal(true);renderInternal(document.querySelector('#page'));toast(product.id?'Suprimento e foto atualizados.':'Suprimento cadastrado com sucesso.');
    }catch(error){
...
function receiptUploadModal(requestId=''){
  document.querySelector('#modal').innerHTML=`<div class="modal"><form class="modal-box receipt-upload" id="receiptUploadForm"><div class="modal-head"><div><p class="eyebrow">NOVO CUPOM</p><h2>Como deseja adicionar o cupom?</h2></div><button type="button" data-close>×</button></div><div class="capture-choice-grid"><label class="capture-choice"><input id="receiptCamera" type="file" accept="image/*" capture="environment"><i>📷</i><b>Tirar foto agora</b><span>Abra a câmera traseira do celular</span></label><label class="capture-choice"><input id="receiptGallery" type="file" accept="image/jpeg,image/png,image/webp"><i>🖼️</i><b>Escolher da galeria</b><span>Use uma imagem já salva no aparelho</span></label></div><div class="capture-selected" id="receiptSelected" role="status"><i>○</i><span>Nenhuma imagem selecionada</span></div><p class="capture-guidance">Use boa luz, enquadre todo o cupom e evite sombras.</p><div class="receipt-ai-info"><i>✨</i><p><b>Leitura inteligente com conferência</b><span>Os dados serão preenchidos automaticamente, mas nada será salvo antes da sua revisão.</span></p></div><div id="receiptAiProgress" hidden></div><div class="form-actions"><button type="button" class="outline" id="manualReceipt">Preencher manualmente</button><button class="primary" id="analyzeReceipt">Ler cupom automaticamente</button></div></form></div>`;
  document.querySelector('[data-close]').onclick=closeModal;
  let selectedReceiptFile=null;
  const receiptInputs=[document.querySelector('#receiptCamera'),document.querySelector('#receiptGallery')],selected=document.querySelector('#receiptSelected'),manual=document.querySelector('#manualReceipt'),analyze=document.querySelector('#analyzeReceipt');
  const progress=window.HarmonyExperience?.createAIProgress(document.querySelector('#receiptAiProgress'),{name:'cupom fiscal'});
  receiptInputs.forEach(input=>input.onchange=()=>{const file=input.files?.[0];if(!file)return;selectedReceiptFile=file;receiptInputs.filter(other=>other!==input).forEach(other=>other.value='');selected.classList.add('ready');selected.innerHTML=`<i>✓</i><span><b>Imagem pronta:</b> ${esc(file.name||'foto do cupom')}</span>`;progress?.reset();analyze.disabled=false;manual.disabled=false});
  manual.onclick=()=>receiptReview({merchant_name:'',merchant_document:'',fiscal_access_key:'',receipt_number:'',purchased_at:new Date().toISOString(),total_value:0,payment_method:'',confidence:0,warnings:[],items:[{description:'',quantity:1,unit:'unidade',unit_price:0,total_price:0,confidence:0}]},'',false,'',{},requestId);
  analyze.onclick=async event=>{event.preventDefault();const button=event.currentTarget,file=selectedReceiptFile;if(!file)return alert('Tire uma foto ou escolha uma imagem do cupom na galeria.');button.disabled=true;manual.disabled=true;receiptInputs.forEach(input=>input.disabled=true);button.textContent='Lendo cupom…';let path='';progress?.start();try{path=await uploadReceipt(file);progress?.phase('prepare');await new Promise(resolve=>setTimeout(resolve,80));progress?.phase('reading');const result=await receiptEdge({image_path:path});progress?.phase('validate');await new Promise(resolve=>setTimeout(resolve,180));await progress?.complete();receiptReview(result.extraction,path,true,result.model,result.usage,requestId)}catch(error){if(path)storageFetch('/storage/v1/object/internal-receipts/'+encodedStoragePath(path),{method:'DELETE'}).catch(()=>{});progress?.fail();alert(error.message);button.disabled=false;manual.disabled=false;receiptInputs.forEach(input=>input.disabled=false);button.textContent='Tentar leitura novamente'}};
}

function bestProduct(description,candidates=IS.products){const term=normalizeSearch(description);if(!term)return'';const words=term.split(/\s+/).filter(word=>word.length>2);const ranked=candidates.map(product=>{const name=normalizeSearch(product.name),nameWords=name.split(/\s+/).filter(word=>word.length>2),shared=words.filter(word=>nameWords.some(nameWord=>nameWord===word||nameWord.includes(word)||word.includes(nameWord))).length,exact=term.includes(name)||name.includes(term)?3:0;return {id:product.id,score:exact+shared}}).sort((a,b)=>b.score-a.score);return ranked[0]?.score?ranked[0].id:''}
function receiptItemRow(item,index,requestId=''){const requested=requestId?requestItems(requestId).map(row=>productBy(row.product_id)).filter(Boolean):[],match=bestProduct(item.description,requested)||bestProduct(item.description,IS.products.filter(product=>product.active));return `<article class="receipt-review-item" data-receipt-row="${index}"><label class="wide">Descrição lida<input name="description" value="${esc(item.description||'')}" required></label><label class="wide">Produto correspondente<select name="product_id"><option value="__new__" ${match?'':'selected'}>＋ Criar novo produto automaticamente</option>${requested.length?`<optgroup label="Itens desta solicitação">${requested.map(product=>`<option value="${product.id}" ${match===product.id?'selected':''}>${esc(product.name)}</option>`).join('')}</optgroup>`:''}<optgroup label="Catálogo interno">${IS.products.filter(product=>product.active&&!requested.some(item=>item.id===product.id)).map(product=>`<option value="${product.id}" ${match===product.id?'selected':''}>${esc(product.name)}</option>`).join('')}</optgroup></select></label><label>Quantidade comprada<input name="quantity" type="number" min=".001" step=".001" value="${item.quantity||1}" required></label><label>Unidade<input name="unit" value="${esc(item.unit||'unidade')}"></label><label>Valor unitário<input name="unit_price" type="number" min="0" step=".0001" value="${item.unit_price||0}"></label><label>Total do item<input name="total_price" type="number" min="0" step=".01" value="${item.total_price||0}"></label><input name="confidence" type="hidden" value="${item.confidence||0}"><button type="button" class="danger" data-remove-receipt-row="${index}">Remover</button></article>`}
...
function receiptReview(extraction,imagePath,aiUsed,model,usage={},requestId=''){
  ...
  document.querySelector('#receiptReviewForm').onsubmit=async event=>{event.preventDefault();const button=event.submitter,form=new FormData(event.target),items=[...document.querySelectorAll('.receipt-review-item')].map(row=>{const selected=row.querySelector('[name="product_id"]').value;return {product_id:selected==='__new__'?null:selected,raw_description:row.querySelector('[name="description"]').value,quantity:num(row.querySelector('[name="quantity"]').value),unit:row.querySelector('[name="unit"]').value,unit_price:num(row.querySelector('[name="unit_price"]').value),total_price:num(row.querySelector('[name="total_price"]').value),confidence:num(row.querySelector('[name="confidence"]').value)}});if(!items.length)return alert('Adicione ao menos um item.');const purchasedAt=form.get('purchased_at');if(unusualReceiptDate(purchasedAt)&&!confirm(`${receiptDateAlert(purchasedAt)}\n\nDeseja confirmar e salvar essa data mesmo assim?`)){purchaseDateInput.focus();return}button.disabled=true;try{await rpc('confirm_internal_purchase_receipt',{p_receipt:{request_id:requestId||null,supplier_id:form.get('supplier_id')||null,merchant_name:form.get('merchant_name'),merchant_document:form.get('merchant_document'),fiscal_access_key:form.get('fiscal_access_key'),receipt_number:form.get('receipt_number'),purchased_at:new Date(purchasedAt).toISOString(),total_value:num(form.get('total_value')),payment_method:form.get('payment_method'),image_path:imagePath,ai_used:aiUsed,ai_model:model,ai_confidence:num(data.confidence),extraction:data},p_items:items});closeModal();IS.loaded=false;await loadData();await loadInternal(true);IS.tab=requestId?'requests':'receipts';renderInternal(document.querySelector('#page'));toast(requestId?'Cupom vinculado e solicitação atualizada.':'Compra direta registrada com sucesso.')}catch(error){alert(error.message);button.disabled=false}};
}

function receiptDetail(item){
  const items=receiptItems(item.id),primary=Boolean(S.profile?.is_primary_admin);document.querySelector('#modal').innerHTML=`<div class="modal"><section class="modal-box supply-modal"><div class="modal-head"><div><p class="eyebrow">COMPRA #${String(item.protocol).padStart(4,'0')}</p><h2>${esc(item.merchant_name)}</h2></div><button data-close>×</button></div><div class="supply-detail-meta"><span><small>Data</small><b>${fmt(item.purchased_at)}</b></span><span><small>Total</small><b>${money(item.total_value)}</b></span><span><small>Registro</small><b>${item.ai_used?'✨ Leitura inteligente':'Manual'}</b></span></div><div class="supply-detail-items">${items.map(row=>`<article><div><b>${esc(productBy(row.product_id)?.name||row.raw_description)}</b><small>${esc(row.raw_description)}</small></div><span>${quantity(row.quantity,row.unit)}</span><span><b>${money(row.total_price)}</b></span></article>`).join('')}</div>${item.image_path?'<button class="outline full" id="viewReceiptImage">Visualizar foto original</button>':''}${isAdmin()&&item.status==='confirmed'?'<button class="danger full" id="reverseReceipt">Cancelar compra e estornar estoque</button>':''}${primary?'<button class="danger full permanent-delete" id="deleteInternalReceipt">🗑️ Excluir definitivamente</button><small class="permanent-delete-note">Use somente para teste ou registro criado por engano. Esta ação não pode ser desfeita.</small>':''}</section></div>`;
  document.querySelector('[data-close]').onclick=closeModal;document.querySelector('#viewReceiptImage')?.addEventListener('click',()=>viewReceiptImage(item.image_path));document.querySelector('#reverseReceipt')?.addEventListener('click',async()=>{const reason=prompt('Informe o motivo do cancelamento:');if(reason===null)return;await action(()=>rpc('admin_reverse_internal_purchase_receipt',{p_receipt_id:item.id,p_reason:reason}),'Compra cancelada e estoque estornado.')});document.querySelector('#deleteInternalReceipt')?.addEventListener('click',()=>deleteReceipt(item));
}
async function deleteReceipt(item){
  const confirmation=prompt(`Esta ação excluirá definitivamente o cupom #${String(item.protocol).padStart(4,'0')} e estornará o estoque. Digite EXCLUIR para continuar:`);if(confirmation!=='EXCLUIR')return;
  const deleteProducts=confirm('Também excluir os produtos criados automaticamente por este cupom que não tenham outro uso?\n\nPara o cupom fictício de teste, escolha OK.');
  const reason=prompt('Informe o motivo da exclusão:','Cupom fictício de teste');if(reason===null||!reason.trim())return alert('Informe o motivo da exclusão.');
  try{const result=await rpc('admin_delete_internal_purchase_receipt',{p_receipt_id:item.id,p_reason:reason.trim(),p_delete_orphan_products:deleteProducts});if(result?.image_path)await storageFetch('/storage/v1/object/internal-receipts/'+encodedStoragePath(result.image_path),{method:'DELETE'}).catch(()=>{});closeModal();IS.loaded=false;await loadData();await loadInternal(true);renderInternal(document.querySelector('#page'));toast(`Cupom excluído definitivamente${result?.deleted_products?` e ${result.deleted_products} produto(s) de teste removido(s)`:''}.`)}catch(error){alert(error.message)}
}
...
function printReport(){return window.HarmonyPrint.printCurrentDocument('internal-supplies-printing')}
...
async function openDirectPurchase(){if(!isAdmin())return;S.view='internal-supplies';renderApp();while(IS.loading)await new Promise(resolve=>setTimeout(resolve,40));if(!IS.loaded)await loadInternal(true);receiptUploadModal('')}
window.HarmonyInternalSupplies=Object.freeze({state:IS,load:loadInternal,report:productReport,openRequest:openRequestFromHub,openDirectPurchase});
})();
