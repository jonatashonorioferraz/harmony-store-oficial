...
function receiptUploadModal(requestId=''){
  ...
  const receiptInputs=[document.querySelector('#receiptCamera'),document.querySelector('#receiptGallery')],selected=document.querySelector('#receiptSelected');
  receiptInputs.forEach(input=>input.onchange=()=>{const file=input.files?.[0];if(!file)return;selectedReceiptFile=file;receiptInputs.filter(other=>other!==input).forEach(other=>other.value='');selected.classList.add('ready');selected.innerHTML=`<i>✓</i><span><b>Imagem pronta:</b> ${esc(file.name||'foto do cupom')}</span>`});
  ...
  document.querySelector('#analyzeReceipt').onclick=async event=>{event.preventDefault();const button=event.currentTarget,file=selectedReceiptFile;if(!file)return alert('Tire uma foto ou escolha uma imagem do cupom na galeria.');button.disabled=true;button.textContent='Analisando cupom…';try{const path=await uploadReceipt(file),result=await receiptEdge({image_path:path});receiptReview(result.extraction,path,true,result.model,result.usage,requestId)}catch(error){alert(error.message);button.disabled=false;button.textContent='Ler cupom automaticamente'}};
}
