...
function bindProduction(){
  ...
  document.querySelectorAll('[data-toggle-collection]').forEach(button=>button.onclick=()=>{const id=button.dataset.toggleCollection,card=document.querySelector(`[data-collection-card="${id}"]`),willExpand=!PR.expandedCollections.has(id);if(willExpand)PR.expandedCollections.add(id);else PR.expandedCollections.delete(id);card?.classList.toggle('is-collapsed',!willExpand);button.setAttribute('aria-expanded',String(willExpand));button.setAttribute('aria-label',(willExpand?'Recolher':'Mostrar')+' itens desta coleta');button.textContent=willExpand?'⌃ Recolher lista':'⌄ Ver itens'});
...
async function deleteModel(id){if(!confirm('Excluir este modelo? Modelos já usados em recebimentos não podem ser excluídos.'))return;try{await rest('finished_product_models?id=eq.'+id,{method:'DELETE'});await refreshProduction('Modelo excluído.')}catch{alert('Este modelo possui recebimentos registrados. Você pode editá-lo e deixá-lo inativo.')}}
...
async function printStatement(id){
  try{
    ...
    document.body.appendChild(area);await window.HarmonyPrint.printCurrentDocument('production-receipt-printing',()=>area.remove());
  }catch(error){alert(error.message)}
}
