...
async function load(force=false){
  ...
  TC.loading=(async()=>{
    ...
    if(canRequest())tasks.push(rpc('list_transfer_center_catalog',{}));
...
async function render(){
  ...
  try{
    ...
    page.innerHTML=`<div class="page transfer-center-page">${hero()}${metrics()}<section class="card transfer-toolbar"><label>Localizar solicitação<input id="transferSearch" type="search" placeholder="Número, produto, cor ou solicitante" value="${esc(TC.query)}"></label><label>Situação<select id="transferStatus"><option value="active" ${TC.status==='active'?'selected':''}>Em andamento</option><option value="history" ${TC.status==='history'?'selected':''}>Concluídas e canceladas</option><option value="all" ${TC.status==='all'?'selected':''}>Todas</option></select></label>${canRequest()?'<button class="primary" id="newTransferRequest">＋ Nova solicitação</button>':''}<button class="outline" id="refreshTransferCenter">↻ Atualizar</button></section><section class="transfer-board">${filtered().map(requestCard).join('')||'<div class="card empty">Nenhuma solicitação corresponde aos filtros.</div>'}</section>${reports()}${recentMovements()}</div>`;
    bind();
...
function openCreate(){
  if(!canRequest())return alert('Seu perfil não possui permissão para criar solicitações na Central de Transferências.');
...
async function createFromPlan(planItemId){
  if(!canPlan())throw new Error('Somente a Gerente de e-commerce ou o ADM principal pode solicitar caixas a partir do Planejamento de Envios.');
...
async function openReservation(requestId){
  ...
  try{
    ...
    modal.innerHTML=`<div class="modal"><form class="modal-box large transfer-reservation" id="transferReservationForm"><div class="modal-head"><div><p class="eyebrow">RESERVA FÍSICA ${request?code(request.protocol):''}</p><h2>Selecionar caixas exatas</h2><span>As caixas mais antigas aparecem primeiro. Cada caixa permanece inteira.</span></div><button type="button" data-transfer-close aria-label="Fechar seleção de caixas">×</button></div><aside><b>Proteção contra dupla reserva</b><span>Ao confirmar, as caixas escolhidas ficam indisponíveis para outras solicitações. Antes de confirmar, clique novamente em uma caixa para retirá-la.</span></aside><div class="transfer-reservation-list">${items.map(reservationItem).join('')}</div><label class="transfer-reservation-notes">Observação da reserva<textarea name="notes" maxlength="1200" placeholder="Opcional"></textarea></label><div class="form-actions transfer-reservation-actions"><span data-transfer-selection-summary>Nenhuma caixa selecionada</span><button type="button" class="ghost" data-transfer-clear-selection disabled>Limpar seleção</button><button type="button" class="outline" data-transfer-close>Voltar</button><button class="primary" data-transfer-reserve-button disabled>Reservar caixas selecionadas</button></div></form></div>`;
    document.querySelectorAll('[data-transfer-close]').forEach(button=>button.onclick=()=>modal.innerHTML='');
    const form=document.querySelector('#transferReservationForm');const update=()=>{let selectedBoxes=0,selectedQuantity=0;form.querySelectorAll('[data-transfer-reservation-item]').forEach(section=>{const required=num(section.dataset.required),already=num(section.dataset.already),selected=[...section.querySelectorAll('input:checked')],added=selected.reduce((sum,input)=>sum+num(input.dataset.quantity),0),total=already+added,pct=Math.min(100,required?Math.round(total*100/required):0);selectedBoxes+=selected.length;selectedQuantity+=added;section.querySelector('[data-transfer-selected]').textContent=qty(total);section.querySelector('[data-transfer-coverage]').style.width=pct+'%';section.dataset.covered=total>=required;section.querySelectorAll('.transfer-box-option').forEach(label=>label.dataset.selected=label.querySelector('input').checked?'true':'false')});const summary=form.querySelector('[data-transfer-selection-summary]'),button=form.querySelector('[data-transfer-reserve-button]'),clear=form.querySelector('[data-transfer-clear-selection]');summary.textContent=selectedBoxes?`${selectedBoxes} ${selectedBoxes===1?'caixa selecionada':'caixas selecionadas'} · ${qty(selectedQuantity)}`:'Nenhuma caixa selecionada';button.disabled=!selectedBoxes;clear.disabled=!selectedBoxes};form.querySelectorAll('input[type="checkbox"]').forEach(input=>input.onchange=update);form.querySelector('[data-transfer-clear-selection]').onclick=()=>{form.querySelectorAll('input[type="checkbox"]:checked').forEach(input=>input.checked=false);update()};update();
...
function reservationItem(item){const missing=Math.max(0,num(item.required_quantity)-num(item.selected_quantity));return `<section class="transfer-reservation-item" data-transfer-reservation-item data-item-id="${item.id}" data-required="${num(item.required_quantity)}" data-already="${num(item.selected_quantity)}"><header><img src="${esc(image(item.image_path))}" alt="${esc(item.model_name)}"><span><b>${esc(item.model_name)}</b><em><i style="--transfer-color:${esc(item.color_hex)}"></i>${esc(item.color_name)}</em></span><strong><small>NECESSÁRIO</small>${qty(item.required_quantity)}<em>Faltam ${qty(missing)}</em></strong></header><div class="transfer-box-options">${(item.boxes||[]).map(box=>`<label class="transfer-box-option" data-selected="false"><input type="checkbox" value="${box.id}" data-quantity="${num(box.quantity)}" aria-label="Selecionar caixa ${esc(box.box_code)}, ${qty(box.quantity)}"><i class="transfer-box-check" aria-hidden="true">✓</i><span><b>${esc(box.box_code)}</b><small>${qty(box.quantity)}</small><em><span>Entrada ${date(box.entry_on)}</span><span>${esc(box.location||'Sem localização')}</span></em></span></label>`).join('')||'<p class="empty">Nenhuma caixa compatível disponível no momento.</p>'}</div><footer><span>Total após a seleção: <b data-transfer-selected>${qty(item.selected_quantity)}</b></span><i aria-hidden="true"><em data-transfer-coverage></em></i></footer></section>`}
...
function openDetail(id){
  ...
  document.querySelectorAll('[data-transfer-release-box]').forEach(button=>button.addEventListener('click',()=>releaseReservedBox(id,button.dataset.transferReleaseBox,button.dataset.boxCode)));
  document.querySelectorAll('[data-transfer-remove-item]').forEach(button=>button.addEventListener('click',()=>removeRequestItem(id,button.dataset.transferRemoveItem,button.dataset.modelName)));
}
function detailItem(item,request){const editable=canPlan()&&['requested','partially_reserved','reserved'].includes(request.status),manual=request.source_type==='manual';return `<article class="transfer-detail-item"><img src="${esc(image(item.image_path))}" alt="${esc(item.model_name)}"><span><b>${esc(item.model_name)}</b><em><i style="--transfer-color:${esc(item.color_hex)}"></i>${esc(item.color_name)}</em><small>Solicitado ${qty(item.required_quantity)} · reservado ${qty(item.selected_quantity)}</small>${editable&&manual?`<button type="button" class="transfer-remove-item" data-transfer-remove-item="${item.id}" data-model-name="${esc(item.model_name)}">Retirar modelo da solicitação</button>`:''}</span><div class="transfer-detail-boxes">${(item.boxes||[]).map(box=>`<span class="transfer-detail-box"><strong>${esc(box.box_code)}<small>${qty(box.quantity)} · ${date(box.entry_on)}</small></strong>${editable?`<button type="button" data-transfer-release-box="${box.id}" data-box-code="${esc(box.box_code)}" aria-label="Liberar caixa ${esc(box.box_code)}">Liberar caixa</button>`:''}</span>`).join('')||'<em>Nenhuma caixa reservada</em>'}</div></article>`}

async function releaseReservedBox(requestId,boxId,boxCode){const reason=prompt(`Informe o motivo para liberar a caixa ${boxCode}:`);if(!reason)return;if(!confirm(`Liberar somente a caixa ${boxCode}? As demais reservas serão mantidas.`))return;try{await rpc('release_transfer_center_box',{p_request_id:requestId,p_inventory_entry_id:boxId,p_reason:reason});TC.loaded=false;await load(true);if(S.view==='transfer-center')await render();openDetail(requestId);toast(`Caixa ${boxCode} liberada e disponível novamente.`)}catch(error){alert(error.message)}}
async function removeRequestItem(requestId,itemId,modelName){const reason=prompt(`Informe o motivo para retirar ${modelName} desta solicitação:`);if(!reason)return;if(!confirm(`Retirar ${modelName} e liberar todas as caixas reservadas para este modelo?`))return;try{await rpc('remove_transfer_center_request_item',{p_request_id:requestId,p_request_item_id:itemId,p_reason:reason});TC.loaded=false;await load(true);if(S.view==='transfer-center')await render();openDetail(requestId);toast(`${modelName} foi retirado sem alterar os demais modelos.`)}catch(error){alert(error.message)}}
