...
function dataQuality(){
  ...
  return`<section class="card shopee-snapshot shopee-quality"><header><div><p class="eyebrow">QUALIDADE DOS DADOS</p><h2>Base confiável</h2></div><button data-shopee-tab="imports">Ver histórico →</button></header><ul><li><b>✓</b><span>${comp.complete} de ${comp.total} período(s) completos</span></li><li><b>✓</b><span>Proteção contra soma duplicada ativa</span></li><li><b>✓</b><span>${imports.length} arquivo(s) no histórico auditável</span></li></ul><footer>${latest?`Última importação: ${datetime(latest.imported_at)}`:'Aguardando primeira importação'}</footer></section>`;
}
...
function bind(){
  ...
  document.querySelectorAll('[data-shopee-open-imports]').forEach(button=>button.onclick=()=>openTab('imports'));
