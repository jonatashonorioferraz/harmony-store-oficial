...
export const STAGE_OUTPUT_SCHEMAS: Partial<Record<StageKey, Record<string, unknown>>> & Record<"triage" | "visual-analysis" | "strategy" | "copy" | "art-direction" | "compliance-review" | "quality-gate", Record<string, unknown>> = {
  ...
  "art-direction": object({ briefs: { type: "array", minItems: 6, maxItems: 6, items: object({ objective: { type: "string" }, composition: { type: "string" }, lighting: { type: "string" }, restrictions: strings }) } }),
  "compliance-review": object({ decision: { type: "string", enum: ["approved", "rejected", "changes_requested"] }, score: { type: "number", minimum: 0, maximum: 100 }, images: { type: "array", minItems: 6, maxItems: 6, items: object({ slot: { type: "integer", minimum: 1, maximum: 6 }, decision: { type: "string", enum: ["approved", "rejected", "changes_requested"] }, score: { type: "number", minimum: 0, maximum: 100 }, issues: strings }) }, issues: strings, corrections: strings }),
  "quality-gate": object({ release: { type: "string", enum: ["approved", "reprocess", "rejected"] }, reprocessStage: { anyOf: [{ type: "string", enum: ["triage", "visual-analysis", "strategy", "copy", "art-direction", "visual-production-1", "visual-production-2", "visual-production-3", "visual-production-4", "visual-production-5", "visual-production-6", "compliance-review"] }, { type: "null" }] }, reasons: strings }),
};
