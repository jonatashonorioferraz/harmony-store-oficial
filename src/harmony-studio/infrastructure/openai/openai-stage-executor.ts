...
export class OpenAIStageExecutor implements StageExecutor {
  private readonly client: OpenAIHttpClient; private readonly budget: UsageBudget; private readonly assets: GeneratedAssetSink;
  constructor(input: { client: OpenAIHttpClient; budget: UsageBudget; assets: GeneratedAssetSink }) { this.client = input.client; this.budget = input.budget; this.assets = input.assets; }
  async execute(context: ContextBundle, idempotencyKey: string): Promise<StageExecutionResult> { return context.stageKey === "visual-production" ? this.image(context, idempotencyKey) : this.structured(context, idempotencyKey); }
  private async structured(context: ContextBundle, idempotencyKey: string) {
    const policy = STAGE_MODEL_POLICY[context.stageKey as keyof typeof STAGE_MODEL_POLICY]; await this.budget.reserve(idempotencyKey, policy.maxEstimatedCostUsd);
    const { payload, requestId } = await this.client.request("responses", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: policy.model, instructions: instructions(context), input: JSON.stringify({ inputs: context.inputs, approvedReferences: context.excellenceReferences }), reasoning: { effort: policy.reasoningEffort }, max_output_tokens: policy.maxOutputTokens, text: { verbosity: "medium", format: { type: "json_schema", name: `${context.stageKey.replaceAll("-", "_")}_output`, strict: true, schema: STAGE_OUTPUT_SCHEMAS[context.stageKey as keyof typeof STAGE_OUTPUT_SCHEMAS] } }, store: false }) }, idempotencyKey);
  ...
  private async image(context: ContextBundle, idempotencyKey: string) {
    await this.budget.reserve(idempotencyKey, IMAGE_MODEL_POLICY.maxEstimatedCostUsd); const images = (context.inputs.assets ?? []) as Array<{ blob?: Blob; name?: string }>;
    if (!Array.isArray(images) || images.length !== 4 || images.some((item) => !(item.blob instanceof Blob))) throw new OpenAIIntegrationError({ kind: "invalid_request", message: "Visual production requires exactly four image blobs" });
