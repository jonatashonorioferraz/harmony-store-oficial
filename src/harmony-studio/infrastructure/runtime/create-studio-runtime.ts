...
export function createStudioRuntime(input: { db: D1DatabasePort; bucket: R2BucketPort; apiKey: string }) {
  ...
  return { storage, knowledge, audit, repository, orchestrator: new WorkflowOrchestrator({ repository, knowledge, excellence: new D1ExcellenceLibraryRepository(input.db), executor, audit }) };
}
