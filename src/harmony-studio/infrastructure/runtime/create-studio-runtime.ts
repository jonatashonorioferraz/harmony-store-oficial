...
import { D1VisualStandardRepository } from "../persistence/d1-visual-standard-repository.ts";
...
export function createStudioRuntime(input: { db: D1DatabasePort; bucket: R2BucketPort; apiKey: string }) {
  ...
  return { storage, knowledge, audit, repository, orchestrator: new WorkflowOrchestrator({ repository, knowledge, excellence: new D1ExcellenceLibraryRepository(input.db), executor, audit, visualReferences: new D1VisualReferenceRepository(input.db), visualStandards:new D1VisualStandardRepository(input.db) }) };
}
