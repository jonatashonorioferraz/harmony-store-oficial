...
import type { GlobalConfigurationReader } from "../ports/global-configuration-reader.ts";

export class StartStudioWorkflow {
  private readonly projects: AdProjectRepository; private readonly audit: AuditEventRepository; private readonly snapshots: ProductSnapshotRepository; private readonly assets: SourceAssetService; private readonly orchestrator: WorkflowOrchestrator; private readonly configuration?: GlobalConfigurationReader;
  constructor(input: { projects: AdProjectRepository; audit: AuditEventRepository; snapshots: ProductSnapshotRepository; assetMetadata: SourceAssetRepository; storage: AssetStorage; orchestrator: WorkflowOrchestrator; configuration?: GlobalConfigurationReader }) { this.projects = input.projects; this.audit = input.audit; this.snapshots = input.snapshots; this.assets = new SourceAssetService(input.storage, input.assetMetadata); this.orchestrator = input.orchestrator; this.configuration = input.configuration; }
  async execute(input: { ownerId: string; facts: Record<string, unknown>; images: File[] }) {
    ...
    const [titleLimit, brandName, minimumQualityScore] = await Promise.all([this.configuration?.getActive("marketplaceTitleLimit", 120) ?? 120, this.configuration?.getActive("brandName", "Harmony Store Oficial") ?? "Harmony Store Oficial", this.configuration?.getActive("minimumQualityScore", 90) ?? 90]);
    const workflowId = crypto.randomUUID(); await this.orchestrator.start({ id: workflowId, projectId, initialData: { marketplace: input.facts.marketplace ?? "Shopee", productCategory: input.facts.productCategory ?? "mini-sabonetes", product: { declaredFacts: input.facts, approvedFacts: input.facts }, assetMetadata: assets, assets: assets.map((asset) => ({ id: asset.id, name: asset.originalName, contentType: asset.contentType })), marketplacePolicy: { titleLimit, noUnprovenClaims: true }, brandRules: { name: brandName, style: "profissional, elegante e fiel" }, artifactMetadata: { minimumQualityScore } }, actorId: input.ownerId, auditId: crypto.randomUUID(), now: timestamp });
