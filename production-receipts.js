...
function groupedCollections(rows=PR.receipts){
  ...
  rows.forEach(item=>{
    const key=item.collection_id||item.id,current=grouped.get(key)||{
      ...
      notes:item.notes,created_at:item.created_at,items:[]
    };
...
function receiptTable(){
  ...
  return `<div class="receipt-collections">${collections.map(collection=>{const closed=collection.items.some(item=>item.closing_id),editable=!closed&&(isAdmin()||role()==='receiver'&&collection.received_by===S.profile.id),total=collection.items.reduce((sum,item)=>sum+pn(item.amount),0);return `<article class="collection-card"><header><div><small>DATA</small><b>${new Date(collection.received_on+'T12:00:00').toLocaleDateString('pt-BR')}</b></div><div><small>COLABORADORA</small><b>${esc(collection.worker_name)}</b></div><div><small>COLETA / CAIXA</small><b>${esc(collection.box_reference||'Não informada')}</b></div><div><small>ITENS</small><b>${collection.items.length}</b></div>${canSeeValues()?`<div><small>VALOR</small><b>${preciseMoney(total)}</b></div>`:''}<span class="badge ${closed?'production-closed':'production-open'}">${closed?'Semana fechada':'Em aberto'}</span>${canReceive()?`<div class="actions">${editable?`<button class="ghost compact-action" data-edit-collection="${collection.id}">Editar coleta</button><button class="danger compact-action" data-delete-collection="${collection.id}">Excluir coleta</button>`:'—'}</div>`:''}</header><div class="table-wrap"><table class="production-table collection-items-table"><thead><tr><th>Modelo</th><th>Cor</th><th>Informada</th><th>Oficial</th><th>Diferença</th>${canSeeValues()?'<th>Valor</th>':''}</tr></thead><tbody>${collection.items.map(item=>`<tr><td><b>${esc(item.model_name)}</b></td><td><span class="color-pill">${esc(item.color)}</span></td><td>${units(item.declared_quantity??item.quantity)}</td><td><strong>${units(item.quantity)}</strong></td><td>${differenceBadge(item)}</td>${canSeeValues()?`<td>${preciseMoney(item.amount)}</td>`:''}</tr>`).join('')}</tbody></table></div><footer><span>Conferido por <b>${esc(collection.receiver_name)}</b></span>${collection.notes?`<span>Observação: ${esc(collection.notes)}</span>`:''}</footer></article>`}).join('')||'<div class="empty">Nenhuma produção recebida nesta semana.</div>'}</div>`;
}

function reportTable(){
  const rows=groupReport();
  if(!isAdmin()||!rows.length)return '';
  return `<section class="card production-section"><div class="card-head"><div><p class="eyebrow">RESUMO DA SEMANA</p><h2>Produção por modelo e cor</h2></div><button class="outline compact-action" id="exportProduction">Exportar Excel</button></div><div class="table-wrap"><table class="production-table"><thead><tr><th>Colaboradora</th><th>Modelo</th><th>Cor</th><th>Informada</th><th>Oficial</th><th>Diferença</th><th>Valor proporcional</th></tr></thead><tbody>${rows.map(row=>`<tr><td>${esc(row.worker_name)}</td><td>${esc(row.model_name)}</td><td>${esc(row.color)}</td><td>${units(row.declared_quantity)}</td><td><strong>${units(row.quantity)}</strong></td><td>${differenceBadge(row)}</td><td>${money(row.amount)}</td></tr>`).join('')}</tbody></table></div></section>`;
}
...
function weeksView(){
  ...
  PR.receipts.forEach(item=>{const current=byWorker.get(item.worker_id)||{worker_id:item.worker_id,worker_name:item.worker_name,quantity:0,amount:0,receipts:0};current.quantity+=pn(item.quantity);current.amount+=pn(item.amount);current.receipts+=1;byWorker.set(item.worker_id,current)});
  if(!isAdmin()){
  ...
  const workers=PR.workers.map(worker=>{const current=byWorker.get(worker.id)||{worker_id:worker.id,worker_name:worker.full_name,quantity:0,amount:0,receipts:0},closing=PR.closings.find(item=>item.worker_id===worker.id);return {worker,current,closing}}).filter(item=>item.current.receipts||item.closing);
...
function receiptModal(receipt={}){
  const items=receipt.items?.length?receipt.items:[{}],selectedWorker=receipt.worker_id||S.profile.id;
  document.querySelector('#modal').innerHTML=`<div class="modal"><form class="modal-box large" id="productionReceiptForm"><div class="modal-head"><div><p class="eyebrow">CONFERÊNCIA DA COLETA</p><h2>${receipt.id?'Editar recebimento':'Novo recebimento'}</h2><span>Adicione todos os modelos recebidos nesta coleta.</span></div><button type="button" data-production-close>×</button></div><div class="form collection-header-fields"><label>Colaboradora<select name="worker_id" required><option value="">Selecione</option>${PR.workers.map(worker=>`<option value="${worker.id}" ${worker.id===selectedWorker?'selected':''}>${esc(worker.full_name)}</option>`).join('')}</select></label><label>Data do recebimento<input name="received_on" type="date" max="${today()}" value="${receipt.received_on||today()}" required></label><label class="wide">Identificação da coleta/caixas<input name="box_reference" maxlength="80" value="${esc(receipt.box_reference||'')}" placeholder="Ex.: Caixas 12 e 13 (opcional)"></label></div><section class="collection-items-editor"><div class="collection-items-head"><div><small>PRODUTOS DA COLETA</small><b>Adicione modelo por modelo</b></div><button type="button" class="outline compact-action" id="addProductionItem">＋ Adicionar outro produto</button></div><div id="productionItems"></div></section><div class="production-calculation conference-result collection-total"><small>RESUMO DO RECEBIMENTO</small><b id="productionCollectionDifference" class="match">Contagem correta</b><span id="productionCollectionTotals">0 itens • 0 un. oficiais</span>${isAdmin()?`<div class="conference-payment"><small>VALOR TOTAL PELA CONTAGEM OFICIAL</small><strong id="productionCalculated">${money(0)}</strong></div>`:''}</div><div class="form"><label class="wide">Observação da conferência<textarea name="notes" placeholder="Opcional: explique avarias ou diferenças encontradas">${esc(receipt.notes||'')}</textarea></label><div class="form-actions"><button type="button" class="outline" data-production-close>Cancelar</button><button class="primary">Salvar recebimento completo</button></div></div></form></div>`;
