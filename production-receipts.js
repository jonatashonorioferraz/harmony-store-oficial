(()=>{
const PR={loaded:false,loading:false,error:'',models:[],colors:[],receipts:[],closings:[],workers:[],tab:'receipts',weekStart:''};
...
async function loadProduction(force=false){
  ...
  try{
    const calls=[rpc('list_finished_product_models',{}),rpc('list_finished_production_receipts',{p_from:bounds.start,p_to:bounds.end,p_worker_id:null}),rpc('list_production_weekly_closings',{p_from:bounds.start,p_to:bounds.end,p_worker_id:null}),rpc('list_finished_production_colors',{})];
    if(canReceive())calls.push(rpc('list_production_workers',{}));
    const [models,receipts,closings,colors,workers]=await Promise.all(calls);
    Object.assign(PR,{models,colors,receipts,closings,workers:workers||[S.profile],loaded:true});
