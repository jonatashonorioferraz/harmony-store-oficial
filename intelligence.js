...
function collaboratorReport(rows=filteredRows()){
  const grouped=new Map();
  S.team.filter(person=>person.role==='collaborator').forEach(person=>{if(!BI.profileId||person.id===BI.profileId)grouped.set(person.id,{person,requests:new Set(),deliveries:new Set(),items:0,products:new Map(),last:null})});
