...
test('assets, offline cache, backup, recovery and documentation are complete',()=>{
  ...
  assert.match(worker,/production-inventory\.css\?v=25\.72/);assert.match(worker,/production-inventory\.js\?v=25\.73/);assert.match(worker,/harmony-store-v25-92-r1/);
  ...
  assert.match(manual,/## Inventário de Produção/);assert.match(technical,/## Modelo de dados/);assert.equal(JSON.parse(pkg).version,'25.92.0');
});
