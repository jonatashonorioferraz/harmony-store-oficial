...
function addListControls(){
  ...
  if(S.view==='new'){
    ...
    const products=S.products.filter(product=>window.HarmonyProductVisibility?.visibleForRequests?window.HarmonyProductVisibility.visibleForRequests(product):product.active&&(S.profile?.role!=='collaborator'||product.hidden_from_collaborators!==true)),items=[...container?.querySelectorAll('.product')||[]];
